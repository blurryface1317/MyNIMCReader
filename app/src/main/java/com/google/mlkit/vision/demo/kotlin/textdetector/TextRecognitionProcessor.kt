...
class TextRecognitionProcessor(
  ...
  override fun onSuccess(text: Text, graphicOverlay: GraphicOverlay) {
    ...
    Log.d(TAG, "On-device Text detection successful")
    val ccPattern = "[0-9]{4}\\s[0-9]{4}\\s[0-9]{4}\\s[0-9]{4}"
    val pattern: Pattern = Pattern.compile(ccPattern)
    val matcher: Matcher = pattern.matcher(text.text)
    if (matcher.find()) {
      Log.e("nimc card number", matcher.group(0))
      println("Pattern matches")
      println(matcher.group(0))
      Toast.makeText(context , "nimc number: ${matcher.group(0)}" , Toast.LENGTH_SHORT).show()
    } else {
